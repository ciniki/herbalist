...
function ciniki_herbalist_herbAdd(&$ciniki) {
    ...
    $rc = ciniki_herbalist_checkAccess($ciniki, $args['tnid'], 'ciniki.herbalist.herbAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    //
    $str = '';
    $str .= (isset($args['dry']) ? ' ' . $args['dry'] : '');
    $str .= (isset($args['tincture']) ? ' ' . $args['tincture'] : '');
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.herbalist.herb', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.herbalist.herb', 'object_id'=>$herb_id));
