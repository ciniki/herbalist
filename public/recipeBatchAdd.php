...
// Description
// -----------
// This method will add a new recipe batch for the tenant.
//
// Arguments
// ---------
// api_key:
// auth_token:
// tnid:        The ID of the tenant to add the Recipe Batch to.
//
// Returns
// -------
// <rsp stat="ok" id="42">
...
function ciniki_herbalist_recipeBatchAdd(&$ciniki) {
    ...
    $rc = ciniki_herbalist_checkAccess($ciniki, $args['tnid'], 'ciniki.herbalist.recipeBatchAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.herbalist.recipebatch', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.herbalist.recipeBatch', 'object_id'=>$batch_id));
